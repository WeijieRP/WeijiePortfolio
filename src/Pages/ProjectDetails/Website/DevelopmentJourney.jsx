...
export default function TimeframeCarousel({
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        entries.forEach((e) => {
          ...
          if (dir === "down") {
            el.style.setProperty("--enter-x", side === "left" ? enterLeft : enterRight);
            el.style.setProperty("--exit-x", side === "left" ? exitRight : exitLeft);
          } else {
            el.style.setProperty("--enter-x", side === "left" ? enterRight : enterLeft);
            el.style.setProperty("--exit-x", side === "left" ? exitLeft : exitRight);
          }
  ...
  return (
    <section className="tf-stage" id={id} ref={stageRef} aria-label="Development timeline" data-flow="down">
      <div className="tf-bg" style={{ backgroundImage: `url(${bgImage})` }} />
      {/* overlay removed so the background stands out */}

      <div className="tf-container">
        <h2 className="tf-title galaxy-title--soft" data-reveal data-side="left">
