// ChallengeSection.jsx — spacing tuned, functionality unchanged
...
export default function ChallengeSection({
  ...
  problemImage = "/assets/VR_ArtWork/EntranceRoom.png",
  problemCaption = "Goals: clear hints, comfy movement, steady performance."
}) {
  const rootRef = useRef(null);

  // Observe [data-ani] and toggle .in-view for slide-in/out
  useEffect(() => {
    const root = rootRef.current;
    if (!root) return;

    const els = root.querySelectorAll("[data-ani]");
    els.forEach((el, i) => el.style.setProperty("--i", String(i))); // small stagger
  ...
  const renderSummary = (s) =>
    Array.isArray(s)
      ? s.map((p, i) => (
          <p key={i} className="ch-summary slide-left" data-ani data-side="left">
            {p}
          </p>
        ))
      : (
  ...
  return (
    <section className="ch-stage" id={id} ref={rootRef} aria-label="VR Escape Room Problem">
      {/* Full background (no blur) */}
      <div
        className="ch-bg"
      ...
      <div className="ch-inner">
        ...
        <div className="ch-left">
          <article className="ch-card slide-left" data-ani data-side="left" role="group" aria-label="Problem statement">
            {eyebrow && (
        ...
        <aside className="ch-right">
          <figure className="ch-figure slide-right" data-ani data-side="right">
            <img
              src={problemImage}
              ...
              decoding="async"
            />
            <figcaption className="ch-cap slide-right" data-ani data-side="right">
              {problemCaption}
            </figcaption>
          </figure>
        </aside>
