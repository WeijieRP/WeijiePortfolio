...
export default function PortfolioCarousel() {
  ...
  const next = useCallback(() => {
    ...
    setDir("right");
    setIndex((p) => (p + 1) % max);
    setTimeout(() => (animLockRef.current = false), TRANSITION_MS);
  }, [max]);
  ...
  const resume = () => { if (!timer.current) timer.current = setInterval(next, AUTOPLAY_MS); };
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      if (["ArrowDown","ArrowRight"].includes(e.key)) { pause(); next(); setTimeout(resume, AUTOPLAY_MS); }
      if (["ArrowUp","ArrowLeft"].includes(e.key))   { pause(); prev(); setTimeout(resume, AUTOPLAY_MS); }
    };
  ...
  return (
    <section
      className="pc-wrap"
      ref={wrapRef}
      style={{ ["--pc-bg"]: 'url(/assets/PortfolioBackgroundImage/moon-6995110.jpg)' }}
      aria-labelledby="pc-title"
    >
      {/* Title */}
      <div className="pc-header">
        <h2 id="pc-title" className="pc-title">Featured Projects</h2>
        <p className="pc-sub">
          A snapshot of my work from production apps, brand systems, and immersive VR.
        </p>
      </div>
      ...
      <div
        ...
        onBlur={resume}
      >
        <button className="pc-arrow left" onClick={prev} aria-label="Previous slide">
          <ChevronLeft size={26} />
        </button>

        <div className="pc-viewport" role="region" aria-roledescription="carousel">
          <div className="pc-track" ref={trackRef}>
            {SLIDES.map((slide, i) => (
              ...
              >
                <div className="pc-img" style={{ backgroundImage: `url(${slide.img})` }} />
        ...
        <button className="pc-arrow right" onClick={next} aria-label="Next slide">
